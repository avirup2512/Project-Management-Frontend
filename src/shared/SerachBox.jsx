...
function SearchBox({ properties, type, addAllProjectMember }) {
    ...
    const renderSelectedUser = () => {
        switch (type) {
            case "project":
                return (
                    projectSelector.project.user.map((su, index) => (
                        renderInnerSelectedUser(su,index)
                    ))
                )
            case "board":
                return (
                    boardSelector.board.user.map((su, index) => (
                        renderInnerSelectedUser(su,index)
                    ))
                )
            default:
            ...
            <div className='col-md-6'>
                    <div className='parentWrapper position-relative'>
                        <div className='d-flex align-center justify-content-space-between'>
                            ...
                            {
                                (type && type == "board") &&
                                <Button onClick={addAllProjectMember1} size="sm">Add all project member</Button>
                            }
                        </div>
