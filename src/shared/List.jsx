...
import {
  ...
  setBoardList,
  setSelectedBoard,
} from "../dashboard/board/BoardSlice";
import {
  Button,
  Form,
  OverlayTrigger,
...
function ListComponent({
  ...
  const selectBoard = (e) => {
    if (e.target.checked) dispatch(setSelectedBoard(item));
    else {
  ...
  return (
    <>
      <div className="list d-flex align-center">
        <div className="listItems">
          {type != "project" && (
            ...
            >
              <Form.Check
                type="checkbox"
                checked={selectedBoard.hasOwnProperty(item.id)}
              />
            </Form.Group>
          )}
        </div>
        <div
          onClick={open}
          className="align-content-center boardName listItems"
        >
          {item?.name}
        </div>
        <div className="users section listItems">
          {Object.entries(users).map((item, index) => (
            <span className="user" key={index}>
              <span className="align-text-top">{item[1].first_name[0]}</span>
            </span>
          ))}
        </div>
        <div className="status listItems">
          {type != "project" &&
            Object.keys(item.tag).length > 0 &&
            Object.entries(item.tag).map((e, i) => {
              const content = e[1].attachInboard ? (
                <OverlayTrigger
                  placement="top"
                  overlay={<Tooltip>{e[1].tagName}</Tooltip>}
                >
                  <span style={getListStyle(e[1].tagColor)}></span>
                </OverlayTrigger>
              ) : (
                ""
              );
          ...
          {type != "project" && (
            <>
              <ProgressBar
                ...
                now={(item.totalCompleteCard / item.totalCard) * 100}
              />
              <span>
                {item.totalCompleteCard}/{item.totalCard}
              </span>
            </>
          )}
        </div>
