...
function CardTag({
  ...
  useEffect(() => {
    ...
    if (saveTagFlag) {
      saveTag({
        ...
        tagColor: tag.color,
        name: tag.name,
        attachInboard: tag.attach_in_board,
      });
      setSaveTag(false);
    } else {
      setTagColor(tag.color.trim());
      setTagName(tag.name);
      setTagAttachInBoard(tag.attach_in_board);
    }
  }, [tag]);
  ...
  const addBoardTaginCard = (id) => {
    saveBoardTaginCard(id);
  };
  return (
    <>
      <section className="d-flex align-content-center justify-content-space-between p-2 b1">
        {!editMode && (
          <>
            ...
            <span className="color" style={getListStyle(tag?.color)}></span>
