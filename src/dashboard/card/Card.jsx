...
function Card({ item, listId, copyCardProps, onClick }) {
  ...
  const menu = useSelector((e) => e.card.menu);
  const [menuProperties, setMenuProperties] = useState({
    items: [
      {
        name: "Copy Card",
        action: () => {
          setMenuShow(false), copyCardProps({ cardId: item.id });
        },
      },
      {
        name: "Delete Card",
  ...
  const onConfirm = async () => {
    ...
    if (deletedCard.status && deletedCard.status == 200) {
      let list = JSON.parse(JSON.stringify(allList));
      let index = list.findIndex((l) => l.id === listId);
      list[index].cards = list[index].cards.filter((e) => {
        return e.id != item?.id;
      });
      dispatch(setAllList(list));
      setMenuShow(false);
      ...
      >
        <div className="d-flex align-center justify-content-flex-start">
          ...
          <i
            ...
            className="bi bi-three-dots cursor-pointer menuIcon"
          ></i>
          <Menu properties={menu} action={menuAction} show={menuShow}></Menu>
        </div>
        <div className="">
              ...
              return (
                <>
                  {e.tagName && (
                    <span
                      className="tagItem"
                      style={getListStyle(e.tagColor || e.color)}
                    >
                      <span className="align-text-bottom">{e.tagName}</span>{" "}
                    </span>
                  )}
                </>
              );
