...
function CardDetails({ closeCall }) {
    ...
    if (card.status && card.status == 200) {
        ...
        setReminderDate(
          new Date(
            dateService.FromUTCToLocal(
              card.data[Object.keys(card.data)[0]]?.reminder_date
            )
          )
        );
      if (card.data[Object.keys(card.data)[0]].due_date)
        setDueDate(
          new Date(
            dateService.FromUTCToLocal(
              card.data[Object.keys(card.data)[0]]?.due_date
            )
          )
        );
    }
  ...
  const completeCard = async (evt) => {
    const card = await cardService.setStatus({
      boardId,
      listId,
      cardId: currentCard.id,
  ...
  const addComment = async (e) => {
    ...
    if (commentAdded.status && commentAdded.status == 200) {
      let currCard = JSON.parse(JSON.stringify(currentCard));
      currCard.comments.push({
        id: commentAdded.data.insertId,
        comment: e,
        user: loggedInUser.data.first_name + " " + loggedInUser.data.last_name,
    ...
    <>
                ...
                <div className="d-flex align-center justify-content-space-between">
                  <Form>
                    <Form.Check
                      isValid={true}
                      type="checkbox"
                      checked={currentCard?.complete == true}
                      onChange={completeCard}
                      className="checkBox"
                    ></Form.Check>
                  </Form>
                  {(!cardNameEdit && (
                    <p
                      ...
                      className="mb-0 p-1 name width-100"
                    >
                      {currentCard?.name}
                    </p>
                  )) ||
                    (cardNameEdit && (
                      <Form.Group className="width-100">
                        <Form.Control
                          autoFocus={true}
                          className="listName width-100"
                          value={currentCard?.name}
                          type="input"
                          onChange={cardNameChange}
                          onBlurCapture={setCardName}
                        ></Form.Control>
                      </Form.Group>
                    ))}
                </div>
                    ...
                    <div className="d-flex align-center justify-content-flex-start">
                      <div className="">
                        ...
                        <DatePicker
                          ...
                          onChange={(date) => setReminderDateCall(date)}
                        />
                      </div>
                      <div className="ms-3">
                        <p>Reminder Date</p>
                        <DatePicker
                          ...
                          showTimeSelect
                          selected={dueDate}
                          onChange={(date) => setDueDateCall(date)}
                        />
                      </div>
                    </div>
                  </div>
                      ...
                      (canDescriptionEdit && (
                        <Form>
                          ...
                          <Form.Control
                            ...
                            type="input"
                            onChange={cardDescriptionChange}
                            onBlurCapture={setCardDescription}
                          ></Form.Control>
                        </Form>
                      ))}
                  </div>
                ...
                <div className="width-100 pe-2">
                  <div className="members mb-3">
                    ...
                    {currentCard &&
                      ...
                      currentCard?.checkList.length > 0 && (
                        <div className="mb-4 mt-4">
                          <ProgressBar now={progress} />
                        </div>
                      )}
                    {currentCard &&
                      currentCard.hasOwnProperty("checkList") &&
                      currentCard.checkList.length > 0 &&
                      currentCard.checkList.map((e) => {
                        return (
                          <>
                            ...
                            <CheckList
                              i={e}
                              deleteChckList={deleteCheckList}
                            ></CheckList>{" "}
                          </>
                        );
                      })}
              ...
              <div className="col-md-12">
                <div className="width-100 pe-2">
                  <div className="members mb-3">
                    ...
                    <hr className="mt-2"></hr>
                    {currentCard &&
                      currentCard.hasOwnProperty("comments") &&
                      ...
                      return (
                        <>
                          {e.editedUserId && (
                            <>
                              <p>
                                <a>{e.full_name}</a> {e.activity}{" "}
                                {e.editedUserFullName}
                              </p>
                              <p>{e.created_date}</p>
                            </>
                          )}
      ...
      {showModal && (
        <FloatingForm
          ...
          allRoles={roles}
          onItemSelect={userAdd}
          onItemRemove={inputLabel == "Tags" ? deleteTag : userRemove}
          inputLabel={inputLabel}
          close={() => {
            setShowModal(false);
          }}
          onSearch={inputLabel == "Tags" ? searchTags : searchMember}
          searchedList={inputLabel == "Tags" ? searchedTag : searchedUser}
          selectedList={
            inputLabel == "Tags" ? currentCard?.tags : currentCard?.users
          }
          showModal={showModal}
          addTag={addTag}
        />
